...
pub fn filter_out_non_aa(seq: &[u8]) -> Vec<u8> {
    ...
    if log::log_enabled!(log::Level::Trace) && filtered.len() < seq.len() {
        ...
        log::trace!(
            "filtered nb non AA letters {}, fraction  {:1.3e}",
            nb_n,
...
/// for any client
pub(crate) fn process_aafile_in_one_block(
    pathb: &PathBuf,
    ...
    match metadata {
        ...
        Err(_) => {
            println!(
                "process_aafile_in_one_block could not get length of file {} ",
                pathb.to_str().unwrap()
            );
            f_len = 100_000_000;
        }
    ...
    let seqwithid = IdSeq::new(
        ...
        SequenceType::SequenceAA(new_seq),
    );
...
pub fn process_aabuffer_in_one_block(
    pathb: &PathBuf,
    bufread: &[u8],
    filter_params: &FilterParams,
) -> Vec<IdSeq> {
    ...
    // we are at end of file, we have one large sequence for the whole file
    log::debug!(
        "decompressed seq for file : {:?}, seq len is : {}, file length : {}",
...
pub fn process_aabuffer_by_sequence(
    pathb: &PathBuf,
    bufread: &[u8],
    filter_params: &FilterParams,
) -> Vec<IdSeq> {
    ...
    while let Some(record) = reader.next() {
        ...
        if seqrec.seq().len() > 0 {
            if !strid.contains("capsid") && !filter_params.filter(&seqrec.seq()) {
                // process sequence if not capsid and not filtered out, in block mode we do not filter any at the moment
                ...
                if !new_seq.is_empty() {
                    ...
                    log::error!(
                        "null encoded sequence ,file : {:?},  record num : {}, sequence id : {}",
                        pathb,
                        record_num,
                        strid
                    );
