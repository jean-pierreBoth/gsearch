...
use sourmash::sketch::minhash::{max_hash_for_scaled, scaled_for_max_hash, KmerMinHash};
...
fn main() -> io::Result<()> {
    ...
    let matches = Command::new("superaai")
        ...
        .arg(
            Arg::new("ref_list")
                ...
                .long("rl")
                .value_name("FILE")
                .help("File containing list of reference protein paths (.faa format, .gz supported)")
                .required(true)
        )
        .arg(
            Arg::new("output")
    ...
    let kmer_size: u32 = matches.get_one::<String>("kmer_size").unwrap().parse().expect("Invalid k-mer size");
    let scaled: u64 = matches.get_one::<String>("scaled").unwrap().parse().expect("Invalid scaled factor");
    let sketch: u32 = matches.get_one::<String>("sketch").unwrap().parse().expect("Invalid sketch size");
    ...
    let results: Vec<String> = query_files.par_iter().flat_map(|query_file| {
        ref_files.par_iter().map(move |ref_file| {
            // Computing MinHash for query file
            let mut query_mh = KmerMinHash::new(scaled, kmer_size, HashFunctions::Murmur64Protein, 42, false, sketch);
            let mut query_reader = parse_fastx_file(query_file).expect("Cannot open query file");
            ...
            let mut ref_mh = KmerMinHash::new(scaled, kmer_size, HashFunctions::Murmur64Protein, 42, false, sketch);
            let mut ref_reader = parse_fastx_file(ref_file).expect("Cannot open reference file");
