...
impl MatchList {
    pub fn new(base_item: ItemDict) -> Self {
        MatchList {
            base_item,
            candidates: Vec::<SequenceMatch>::new(),
        }
    }
...
struct GenomeMatchAnalyzer {} // end of GenomeMatchAnalyzer

impl GenomeMatchAnalyzer {
    pub fn new() -> Self {
        GenomeMatchAnalyzer {}
    }
...
fn compute_genome_length(sequences: &SeqDict) -> HashMap<String, usize> {
    ...
    for seq in &sequences.0 {
        let entry = hashed_length
            .entry(seq.get_id().get_path().to_string())
            .or_insert(0);
        *entry += seq.get_len();
    }
    //
...
impl Matcher {
    ...
    pub fn insert_sequence_match(&mut self, req_item: ItemDict, new_matches: Vec<SequenceMatch>) {
        ...
        if self.seq_matches.get_mut(req_genome_path).is_none() {
            ...
            log::trace!(
                "insert_sequence_match , creating entry for request genome {}",
                req_genome_path
            );
            self.seq_matches.insert(
                req_genome_path.clone(),
                HashMap::<TargetGenome, MatchList>::new(),
            );
        }
        let genome_for_insertion = self.seq_matches.get_mut(req_genome_path).unwrap();
        ...
        for new_match in &new_matches {
            ...
            if genome_for_insertion.get(candidate_path).is_none() {
                ...
                log::trace!(
                    "insert_sequence_match , creating entry for target genome {}",
                    candidate_path
                );
                genome_for_insertion.insert(
                    candidate_path.to_string(),
                    MatchList::new(candidate_item.clone()),
                );
            }
    ...
    pub fn get_genome_match_mut(
        &mut self,
        request_path: &RequestGenome,
    ) -> Option<&mut HashMap<TargetGenome, MatchList>> {
    ...
    pub fn analyze(&mut self) -> Result<(), String> {
        ...
        if outfile.is_err() {
            log::error!(
                "Matcher analyze : dump could not open file {:?}",
                outpath.as_os_str()
            );
            println!(
                "Matcher analyze: could not open file {:?}",
                outpath.as_os_str()
            );
            panic!("{:?}", "Opening of Matcher output failed");
        }
        let mut outfile = BufWriter::new(outfile.unwrap());
        log::info!(
            "dumping sorted match in : {}, threshold dist : {} ",
            outname,
            threshold
        );
        ...
        for (genome, candidates) in self.seq_matches.iter_mut() {
            ...
            for i in 0..max_out {
                write!(
                    outfile,
                    "\n\t matched genome {}  merit : {:.3E}",
                    sorted_match[i].0, sorted_match[i].1
                )
                .unwrap();
            }
        }
